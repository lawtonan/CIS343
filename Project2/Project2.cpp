...
bool Concert::operator<(const Concert& other) const {
	// compares each variable from tm. If they are smaller it returns
	// true, if it is larger it returns false, otherwise move onto
	// the next comparison. Finally checks which has the greater 
	// desire if the date is the same
...
/**************************************************************************
 * An operator overloader for << to allow printing of the Concert Object
 *
 * @param os "A std::ostream reference"
 * @param ct "A constant Concert reference"
 * @return std::ostream&
 *************************************************************************/
std::ostream& operator<<(std::ostream& os, const Concert& ct) {
	// Adds the correct variable to print to osstream
	// Formatted by NAME MONTH/DAY/YEAR
	os << ct.getName() << " " << ct.getDate().tm_mon << "/" <<
		ct.getDate().tm_mday << "/" << ct.getDate().tm_year;
	return os;
}
/**************************************************************************
 * The main function of Project2. This creates 10 objects and sorts them 
 * via the std::sort
 *
 * @return integer
 *************************************************************************/
int main() {
	// A default vector used in all objects
	std::vector<std::string> buddies;
	
	//Each date is hard coded and has a different date and desire.
	std::tm date1;
	date1.tm_year = 2017;
	date1.tm_mon = 11;
	date1.tm_mday = 13;
	...
	// A vector to contain all the concert objects
	std::vector<Concert::Concert> concertList;

	// Adding all the objects to the vector
